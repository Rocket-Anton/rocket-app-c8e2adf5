...
export function AIAssistant({ open, onClose, onShowAddresses }: AIAssistantProps) {
  ...
  return (
    <>
      ...
      {open && (
        <div className="fixed bottom-6 right-6 w-[440px] h-[600px] flex flex-col bg-gradient-to-br from-background via-background to-primary/5 rounded-2xl shadow-2xl border z-50 animate-in slide-in-from-bottom-4 fade-in duration-300">
          {/* Header */}
          ...
          <ScrollArea className="flex-1 px-4" ref={scrollRef}>
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center py-8">
                <div className="w-16 h-16 rounded-full bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center mb-4 shadow-lg">
                  <Bot className="h-8 w-8 text-primary" />
                </div>
                <h3 className="font-semibold text-lg mb-2">Wie kann ich helfen?</h3>
                <p className="text-xs text-muted-foreground mb-4 px-4">
                  Stelle mir Fragen zu deinen Adressen per Text oder Sprache
                </p>
                <div className="grid grid-cols-1 gap-2 w-full">
                  {[
                    "Zeige Adressen in der Hauptstraße",
                    "Adressen mit Status Potenzial",
                    "Suche PLZ 51063",
                    "Alle Neukundenadressen"
                  ].map((example, i) => (
                    <button
                      key={i}
                      onClick={() => setInput(example)}
                      className="text-xs text-left p-2.5 rounded-lg bg-muted/50 hover:bg-muted transition-colors border border-transparent hover:border-primary/20"
                    >
                      💬 {example}
                    </button>
                  ))}
              ...
              <div className="space-y-3 py-4">
                ...
                {isLoading && (
                  <div className="flex gap-2 justify-start animate-in fade-in-50">
                    <div className="flex-shrink-0">
                      <div className="w-7 h-7 rounded-full bg-gradient-to-br from-primary/20 to-primary/10 flex items-center justify-center">
                        <Bot className="h-3.5 w-3.5 text-primary" />
                      </div>
                    </div>
                    <div className="rounded-2xl px-3 py-2 bg-muted/80">
